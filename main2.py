...
def get_stock_data():
    ...
    fig.savefig('static/stock_data.png')
    ...
    if stock_data['Close'][-1] > stock_data['MA50'][-1] and stock_data['MA50'][-1] > stock_data['MA200'][-1]:
        trend = f"{stock_symbol} is currently in an uptrend."
    elif stock_data['Close'][-1] < stock_data['MA50'][-1] and stock_data['MA50'][-1] < stock_data['MA200'][-1]:
        trend = f"{stock_symbol} is currently in a downtrend."
    else:
        trend = f"{stock_symbol} is currently in a sideways trend."
